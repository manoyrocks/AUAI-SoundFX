...
## Production build and offline support

The app ships as an installable, offline-capable PWA: web manifest, icons
(generated by `apps/web/scripts/gen-icons.mjs` — a dependency-free PNG
encoder, so no image library is needed), and a hand-written service worker.

Two non-obvious bugs were found and fixed while verifying this, both of the
kind that pass a casual test and fail a real user:

1. **Hashed bundles were never precached.** On a first visit the browser
   fetches the JS and CSS *before* the service worker takes control, so its
   fetch handler never observes them. The app reported offline support while
   caching only a shell with no bundle; it self-healed on a second visit. A
   Vite plugin now emits `precache-manifest.json` with the content-hashed
   filenames, which the worker precaches explicitly during install.
2. **`Vary: Origin` broke every cache lookup.** Static hosts commonly send
   it (Vite's own preview server does). Browser-issued module-script and
   stylesheet requests are CORS-mode and carry an `Origin` header; the
   worker's own `cache.add()` requests do not, so matching failed for the
   exact requests the entries were cached to serve. The worker now matches
   with `ignoreVary: true`, which is safe because every entry is a
   same-origin, content-hashed static file — the URL fully determines the
   bytes.

Verified offline end-to-end: with the server stopped, the app loads, mounts,
and synthesises audio from the cached worklet.

## Running on Android

There is no native Android project in this repository, and a WebView wrapper
is not a viable path on older hardware. Verified on a Galaxy Tab S2
(Android 7.0, API 24):

| Runtime | Version | AudioWorklet (needs 66+) |
|---|---|---|
| System WebView | Chrome 64 | **No** |
| Chrome browser | Chrome 119 | Yes |

A Capacitor or Cordova wrapper uses the *system WebView*, so on this device
it would produce an app that cannot make sound at all — the audio engine is
the product. The app runs correctly in Chrome on the same device.

Two consequences for anyone planning mobile distribution:

- **A Trusted Web Activity is the right APK strategy**, not a WebView
  wrapper: a TWA runs the site in the installed Chrome, so it inherits
  Chrome's capabilities rather than the system WebView's. It requires HTTPS
  hosting and Digital Asset Links verification, so it depends on a real
  deployment rather than a local build.
- **Background audio needs native work regardless.** A wind-down or sleep
  session that stops when the screen locks is useless, and keeping audio
  alive requires a foreground service, wake lock, and audio-focus handling.
  That is genuine native development, not a packaging step.

For local testing on a device, `adb reverse tcp:4173 tcp:4173` is the right
tool: it makes the device treat the host's server as `http://localhost`,
which is a secure context. A LAN IP is not, and would silently disable both
the service worker and camera access.

Verified on-device: session runs, audio synthesises (RMS 0.018), the
Vibration API drives the haptic breath pacer, the tablet-portrait two-up
layout renders correctly, and the app continues running with audio after the
tunnel is dropped — i.e. fully from the service-worker cache.
